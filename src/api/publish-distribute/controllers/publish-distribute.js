...
module.exports = createCoreController('api::publish-distribute.publish-distribute', ({ strapi }) => ({
  ...
  async findOne(ctx) {
    ...
    if (!entity) return ctx.notFound('Published release not found');

    return { data: entity };
  },
  async getOnlyPriority(ctx) {
    try {
      const results = await strapi.db.query('api::publish-distribute.publish-distribute').findMany({
  ...
  async delete(ctx) {
    ...
    const deleted = await strapi.entityService.delete('api::publish-distribute.publish-distribute', id);
    ...
    try {
      ...
      if (Array.isArray(payload.tracks)) {
        for (const track of payload.tracks) {
          ...
          if (JSON.stringify(existingTrack.RoleCredits) !== JSON.stringify(mergedCredits)) {
